...
export default class PlaylisterView {
    ...
    refreshPlaylist(playlist) {
        ...
        for (let i = 0; i < playlist.songs.length; i++) {
            ...
            itemDiv.id = "playlist-card-" + (i + 1);
             ...
             // PUT THE NUMBER OF THE SONG INTO THE CARD
            ...
            //CREATING THE SONG HYPERLINK
            var aTag = document.createElement('a');
            aTag.setAttribute('href', "https://youtube.com/watch?v=" + song.youTubeId);
            aTag.appendChild(itemText);
            itemDiv.appendChild(aTag);

            //CREATING REMOVE SONG BUTTON
            var removeSongButton = document.createElement("BUTTON");
            var removeSongContent = document.createTextNode("X");
            removeSongButton.appendChild(removeSongContent);
            itemDiv.appendChild(removeSongButton);
            removeSongButton.setAttribute("class", "list-card-button");
            removeSongButton.setAttribute("id", "delete-" + itemDiv.id);
    ...
    */
    updateToolbarButtons(model) {
        ...
        //THERE IS EITHER A LIST OPEN OR NO LIST IS OPEN, CHANGE BUTTONS ACCORDINGLY
        if((model.hasCurrentList())){
           this.disableButton("add-list-button");
           this.enableButton("close-button");
           this.enableButton("add-button");
           this.updateRedoButton(tps);
           this.updateUndoButton(tps);
        }else{
        ...
        //IF A MODAL(DIALOG) IS OPEN, OR PLAYLIST NAME IS BEING EDITED, 
        //DISABLE CERTAIN BUTTONS TO AVOID TEMPTING THE USER
        //FROM TRYING TO DO ACTIONS THAT ARE NOT ALLOWED
        if (model.confirmDialogOpen || model.renamePlayList) {
            this.disableButton("add-list-button");
            this.disableButton("add-button");
            this.disableButton("undo-button");
            this.disableButton("redo-button");
...
/*
    ...
    //IF A LIST IS OPEN, CHECK IF THERE ARE TRANSACTIONS THAT CAN BE REDONE
    //IF NECESSARY, ENABLE THE REDO BUTTON
    updateRedoButton(tps){
        /*
    ...
    //IF A LIST IS OPEN, CHECK IF THERE ARE TRANSACTIONS THAT CAN BE UNDONE
    //IF NECESSARY, ENABLE THE UNDO BUTTON
    updateUndoButton(tps){
        /*
        if(!this.model.hasCurrentList()){
            return;
        }
